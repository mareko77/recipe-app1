const handleAddRecipe = (req, res, db) => {
    const { userId, name, description, ingredients, url_photo } = req.body;
    if (!userId || !name || !description || !ingredients) {
      return res.status(400).json("Incomplete recipe data");
    }
  
    db("recipes")
      .insert({ user_id: userId, name, description, ingredients, url_photo })
  ...
  const handleUpdateRecipe = (req, res, db) => {
    const { id } = req.params;
    const { name, description, ingredients, url_photo } = req.body;
  
    db('recipes')
      .where({ id })
      .update({ name, description, ingredients, url_photo })
